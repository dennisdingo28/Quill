...
import { ChevronDown, ChevronUp, Loader, Loader2, RotateCw, Search } from "lucide-react";
          ...
          <SimpleBar autoHide={false} className="max-h-[calc(100vh-10rem)]">
            <div ref={ref}>
                ...
                {isLoading && renderedScale ? 
                  <Page width={width ? width:1} rotate={rotation} scale={scale} pageNumber={currPage} key={"@"+renderedScale}/>
                  :
                  null
                }
                <Page className={cn(isLoading ? "hidden":"")} width={width ? width:1} key={"@"+scale} rotate={rotation} scale={scale} pageNumber={currPage} loading={
                  <div className="flex justify-center">
                    <Loader2 className="my-24 h-6 w-6 animate-spin"/>
                  </div>
                }
